...
$ReportFileName                 = "$pwd\Reports\EOL_Orig.md" # "$pwd\Reports\Office365ABAC_$($(Get-Date).ToString(`"yyyy-MM-dd hhmmss`")).md"
...
$domains |Foreach-Object {
    ...
    Else{
        If ($EOMXrecords -isnot [array]) {
            $EOMXrecordPreference = $EOMXrecords.Preference
            $EOMXrecordMX = $EOMXrecords.NameExchange
            $EOMXrecordsDetail = [ordered]@{
                "Domain"              = $_ 
...
If ($null -ne $EOAuthenticationPolicy) {
